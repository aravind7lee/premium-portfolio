        ...
        return (
          <div
            ...
            style={{
              ...
              transform: containerVisible ? "translate3d(0,0,0)" : "translate3d(0,8px,0)",
            }}
          >
            {child}
          </div>
...
export default function Skills() {
  ...
  // defensive getters
  const frontendSkills = Array.isArray(skills?.frontend) ? skills.frontend : [];
  const backendSkills = Array.isArray(skills?.backend) ? skills.backend : [];
    ...
    <>
      ...
      >
        <div className="absolute inset-0 -z-10 overflow-hidden">
          ...
          <div
            className={`absolute -bottom-40 -left-40 w-80 h-80 bg-teal-500/10 rounded-full ultra-deco ultra-blur ${
              ULTRA_SMOOTH && isDesktop ? "ultra-pulse delay-1000" : "animate-pulse-slow delay-1000"
            }`}
            aria-hidden
          ></div>
          ...
          <div
            className={`absolute top-1/2 left-1/4 w-60 h-60 bg-indigo-500/5 rounded-full ultra-deco ultra-blur ${
              ULTRA_SMOOTH && isDesktop ? "ultra-pulse delay-2000" : "animate-pulse-slow delay-2000"
            }`}
            aria-hidden
          ></div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal variant="fade-down" duration={0.44} delay={0.08} className="text-center mb-16">
            <div>
              <h1
                id="skills-heading"
            ...
            <Reveal variant="fade-right" duration={0.44} delay={0.12}>
              <div className="rounded-2xl p-8 transition-all duration-300 border bg-white/80 supports-[backdrop-filter]:bg-white/70 border-gray-200/70 shadow-lg relative overflow-hidden dark:bg-gray-900/70 dark:border-gray-700/50 group card-backdrop card-shadow skill-card">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <StaggerReveal staggerDelay={0.05} variant="fade-up" duration={0.36}>
                    {frontendSkills.length ? (
                      frontendSkills.map((skill, index) => (
                        <div key={skill.name ?? `frontend-${index}`} className="p-2">
                          <SkillItem skill={skill} index={index} />
                        </div>
                      ))
                    ) : (
                      <div className="p-4 text-sm text-gray-600 dark:text-gray-300">No frontend skills found.</div>
                    )}
                  </StaggerReveal>
                </div>
              </div>
                  ...
                  <StaggerReveal staggerDelay={0.05} variant="fade-up" duration={0.36}>
                    {backendSkills.length ? (
                      backendSkills.map((skill, index) => (
                        <div key={skill.name ?? `backend-${index}`} className="p-2">
                          <SkillItem skill={skill} index={index} />
                        </div>
                      ))
                    ) : (
                      <div className="p-4 text-sm text-gray-600 dark:text-gray-300">No backend skills found.</div>
                    )}
                  </StaggerReveal>
                </div>
              </div>
            ...
            <div className="mt-16 text-center">
              <div className="rounded-2xl p-8 border bg-white/80 backdrop-blur-xl supports-[backdrop-filter]:bg-white/70 border-gray-200/70 shadow-lg relative overflow-hidden dark:bg-gray-900/70 dark:border-gray-700/30 group">
                ...
                <Reveal variant="fade-down" duration={0.36} delay={0.58}>
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Continuous Learning & Growth</h3>
                </Reveal>
                ...
                <Reveal variant="fade-up" duration={0.36} delay={0.66}>
                  <p className="text-gray-700 dark:text-gray-300 max-w-3xl mx-auto leading-relaxed">
                    I'm passionate about staying up-to-date with the latest technologies and best practices. These skills represent my current expertise, but I'm always expanding my knowledge through new projects, courses, and hands-on experience.
                  </p>
                </Reveal>
          ...
          <div className="absolute left-[10%] top-[30%] -translate-y-1/2 w-72 h-72 bg-purple-500/5 rounded-full blur-3xl -z-10 ultra-deco ultra-blur" aria-hidden></div>
          <div className="absolute right-[15%] bottom-[20%] translate-y-1/2 w-96 h-96 bg-teal-500/5 rounded-full blur-3xl -z-10 ultra-deco ultra-blur" aria-hidden></div>
        </div>
      </Motion.main>
    </>
  );
